...
public class ListeImmeublesFavoriActivity extends Activity implements
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		listview.setOnItemClickListener(new OnItemClickListener() {
			public void onItemClick(AdapterView<?> parent, View view,
				...
				Intent intent_POI = new Intent(
						ListeImmeublesFavoriActivity.this, POIActivity.class);
	...
	public void onClick(View v) {
		if (v == bouton1) {
			Toast.makeText(getApplicationContext(), "Liste des b�timents",
					Toast.LENGTH_SHORT).show();
			Intent intent_Immeuble = new Intent(
					ListeImmeublesFavoriActivity.this,
					ListeImmeublesActivity.class);
			startActivity(intent_Immeuble);
		}
		if (v == bouton2) {
		...
		if (v == bouton3) {
			Toast.makeText(getApplicationContext(),
					"Vous êtes déjà sur la liste des favoris",
					Toast.LENGTH_SHORT).show();
		}
	}
	...
	public void afterTextChanged(Editable recherche) {
		...
		for (Immeuble Immeuble : Database.getImmeuble()) {
			if (Immeuble.getFavoris()
					&& Immeuble.getNom().toLowerCase()
							.contains(recherche.toString())) {
				listImmeublesaffiche.add(Immeuble);
			}
		}
